# --- START OF FILE db_profiles/mimic_iv/panels/diagnosis_panel.py ---
...
class DiagnosisConfigPanel(BaseSourceConfigPanel):
    ...
    def populate_panel_if_needed(self):
        ...
        # 诊断通常只关注 "住院以前" 作为既往史，或在当前住院期间发生的
        diag_event_time_options = ["住院以前 (既往史)", "整个住院期间 (当前入院)"]
    ...
    def get_panel_config(self) -> dict:
        ...
        event_time_column = None # diagnoses_icd 表没有自己的事件时间戳

        # 对于“住院以前”，需要特殊的JOIN逻辑来比较入院时间
        ...
        return {
            ...
            "item_filter_conditions": (condition_sql, condition_params), # For logging/reproducibility
        }
    ...
    def _get_primary_item_label_for_naming(self):
        if self.item_list.selectedItems():
            ...
            return first_selected_item_text.split(' (ICD Code:')[0].strip()
        return None
    ...
    def _filter_items_action(self):
        ...
        dict_table, name_col, id_col = "mimiciv_hosp.d_icd_diagnoses", "long_title", "icd_code"
