# --- START OF FULLY CORRECTED FILE: db_profiles/mimic_iv/panels/diagnosis_panel.py ---
...
class DiagnosisConfigPanel(BaseSourceConfigPanel):
    def init_panel_ui(self):
        ...
        filter_group = QGroupBox("筛选诊断 (来自 mimc_hosp.d_icd_diagnoses)"); filter_group_layout = QVBoxLayout(filter_group); filter_group_layout.setSpacing(8)
        ...
        self.filter_items_btn = QPushButton("筛选诊断项目"); self.filter_items_btn.clicked.connect(self._filter_items_action); filter_action_layout.addWidget(self.filter_items_btn); filter_group_layout.addLayout(filter_action_layout)
        ...
        self.filter_sql_preview_textedit = QTextEdit(); self.filter_sql_preview_textedit.setReadOnly(True); self.filter_sql_preview_textedit.setFixedHeight(60); self.filter_sql_preview_textedit.setPlaceholderText("执行“筛选诊断项目”后将在此显示SQL..."); filter_group_layout.addWidget(self.filter_sql_preview_textedit)
    ...
    def _filter_items_action(self):
        ...
        dict_table, name_col, id_col, ver_col = "mimiciv_hosp.d_icd_diagnoses", "long_title", "icd_code", "icd_version"
