...
1.  **数据库**:
    *   **DB1**: PostgreSQL 服务器，包含 **MIMIC-IV v2.2** 数据库（例如，库名 `mimiciv`）。
    *   **DB2**: PostgreSQL 服务器，包含 **MIMIC-IV v3.1** 数据库（例如，库名 `mimiciv3`）。
...
这个测试计划应该能覆盖您应用的大部分核心功能。您可以根据这个模板进行测试，并在测试过程中记录实际结果。祝您测试顺利！