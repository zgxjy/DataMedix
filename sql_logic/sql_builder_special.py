...
def build_special_data_sql(
    ...
    if quick_extractors:
        for key, pattern in quick_extractors.items():
            ...
            is_valid, err = validate_column_name(final_col_name)
            if not is_valid: return None, f"生成的列名 '{final_col_name}' 无效: {err}", [], []
            sql_template_with_placeholder = "(REGEXP_MATCHES({val_col}, %s, 'i'))[1]"
    ...
    for _, final_col_ident, agg_template_or_tuple, _ in selected_methods_details:
        ...
        if isinstance(agg_template_or_tuple, tuple):
            ...
            sql_expr = psql.SQL("MAX({})").format(sql_template)
    ...
    if for_execution:
        ...
        preview_sql = psql.SQL("WITH MergedDataCTE AS ({data_gen_query}) SELECT {select_cols} FROM {target_table} {coh_alias} LEFT JOIN MergedDataCTE {md_alias} ON {coh_alias}.{join_key} = {md_alias}.{join_key} ORDER BY RANDOM() LIMIT {limit};").format(
            data_gen_query=data_gen_query_part, select_cols=psql.SQL(', ').join(preview_select_cols),
            target_table=target_table_ident, coh_alias=cohort_alias,
            md_alias=md_alias, join_key=group_by_key_ident, limit=psql.Literal(preview_limit)
        )
